import 'package:aplicacion_tenis/screens/tournaments/semis_screen.dart';
import 'package:aplicacion_tenis/screens/tournaments/update_scores_screen.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';

class CuartosScreen extends StatefulWidget {
  final String torneo;

  CuartosScreen({required this.torneo});

  @override
  _CuartosScreenState createState() => _CuartosScreenState();
}

class _CuartosScreenState extends State<CuartosScreen> {
  ...
  void _borrarResultados() {
    ...
    // Borrar los emparejamientos y resultados generados en Octavos
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              'Se han borrado los resultados y emparejamientos de Octavos.')),
    );
  }
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(
            '${widget.torneo} - Cuartos de Final',
            ...
            FutureBuilder(
              ...
              builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                ...
                final emparejamientos =
                    torneoData['Emparejamientos cuartos'] as List<dynamic>;
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          if (_todosLosCamposLlenos()) {
                            await _guardarResultadosCuartos(
                                context, torneoId, emparejamientos);
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) =>
                                    SemisScreen(torneo: widget.torneo),
                              ),
                            );
                          } else {
                            ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        child: Text('Pasar a Semifinales'),
  ...
  Future<void> _guardarResultadosCuartos(BuildContext context, String torneoId,
      List<dynamic> emparejamientos) async {
    final ganadores = <String>[];
    final puntuacionesCuartos = <String, Map<String, int>>{};
    ...
    // Generar emparejamientos para semifinales
    final emparejamientosSemis = _generarEmparejamientos(ganadores);

    await FirebaseFirestore.instance
        .collection('Torneos')
        .doc(torneoId)
        .update({
      'Puntuaciones cuartos': puntuacionesCuartos,
      'Ganadores cuartos': ganadores,
      'Emparejamientos semis': emparejamientosSemis,
    });

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              'Resultados guardados y emparejamientos de semifinales creados.')),
    );
  }
